...
const ChartTooltip = ({ active, payload, label }) => {
  ...
  return (
    <div style={{ background: "#1a1d27", border: "1px solid #2e3248", borderRadius: 8, padding: "10px 14px", fontSize: 12 }}>
...
const PctLabel = ({ x, y, width, value, fill }) => (
  value > 0 ? (
    <text x={x + width + 5} y={y + 10} fill="#8892a4" fontSize={11}>{value}%</text>
  ) : null
);
  ...
  return (
    <ResponsiveContainer width="100%" height={h}>
      <BarChart data={data} layout="vertical" margin={{ left: 8, right: 60, top: 4, bottom: 4 }}>
        <CartesianGrid horizontal={false} strokeDasharray="3 3" stroke="#2e3248" />
        <XAxis type="number" tick={{ fill: "#5a6480", fontSize: 11 }} axisLine={false} tickLine={false} />
        <YAxis type="category" dataKey={nameKey} tick={{ fill: "#8892a4", fontSize: 12 }} width={220} axisLine={false} tickLine={false} />
        <Tooltip content={<ChartTooltip />} />
        <Bar dataKey={dataKey} fill={color} radius={[0, 3, 3, 0]} maxBarSize={20}>
          {showPct && <LabelList dataKey="pct" position="right" style={{ fill: "#8892a4", fontSize: 11 }} formatter={(v) => `${v}%`} />}
        </Bar>
      </BarChart>
    </ResponsiveContainer>
  );
  ...
  return (
    <ResponsiveContainer width="100%" height={h}>
      <BarChart data={data} layout="vertical" margin={{ left: 8, right: 20, top: 4, bottom: 4 }}>
        <CartesianGrid horizontal={false} strokeDasharray="3 3" stroke="#2e3248" />
        <XAxis type="number" tick={{ fill: "#5a6480", fontSize: 11 }} axisLine={false} tickLine={false} />
        <YAxis type="category" dataKey={nameKey} tick={{ fill: "#8892a4", fontSize: 12 }} width={200} axisLine={false} tickLine={false} />
        <Tooltip content={<ChartTooltip />} />
        <Legend wrapperStyle={{ fontSize: 12, color: "#8892a4" }} />
        {keys.map((k, i) => (
  ...
  return (
    <ResponsiveContainer width="100%" height={Math.max(data.length * 36 + 60, 200)}>
      <BarChart data={chartData} layout="vertical" margin={{ left: 8, right: 20, top: 4, bottom: 4 }}>
        <CartesianGrid horizontal={false} strokeDasharray="3 3" stroke="#2e3248" />
        <XAxis type="number" tick={{ fill: "#5a6480", fontSize: 11 }} axisLine={false} tickLine={false} />
        <YAxis type="category" dataKey="label" tick={{ fill: "#8892a4", fontSize: 11 }} width={220} axisLine={false} tickLine={false} />
        <Tooltip content={({ active, payload, label }) => {
          ...
          return (
            <div style={{ background: "#1a1d27", border: "1px solid #2e3248", borderRadius: 8, padding: "10px 14px", fontSize: 12 }}>
              ...
              <div style={{ color: "#e2e8f0", borderTop: "1px solid #2e3248", marginTop: 4, paddingTop: 4 }}>Net: <b>{d?.net > 0 ? "+" : ""}{d?.net}</b></div>
            </div>
          );
        }} />
        <Legend wrapperStyle={{ fontSize: 12, color: "#8892a4" }} />
...
const RoutineJourney = ({ data }) => (
  <div style={{ display: "flex", flexDirection: "column", gap: 0 }}>
    ...
    <div style={{ display: "grid", gridTemplateColumns: "40px 1fr 120px 120px", gap: 12, marginBottom: 12, padding: "0 16px" }}>
      <div />
      <div style={{ fontSize: 12, fontWeight: 600, color: "#8892a4" }}>단계</div>
          ...
          {step.am ? (
            <div>
              <div style={{
                ...
                background: `linear-gradient(to right, #fb923c ${step.pct_am}%, #2e3248 ${step.pct_am}%)`,
              }} />
              <span style={{ fontSize: 12, color: "#fb923c", fontWeight: 600 }}>{step.pct_am}%</span>
            </div>
          ) : <span style={{ color: "#2e3248" }}>—</span>}
        </div>
        <div style={{ textAlign: "center" }}>
          {step.pm ? (
            <div>
              <div style={{
                height: 6, borderRadius: 3, marginBottom: 4,
                background: `linear-gradient(to right, #6c7aff ${step.pct_pm}%, #2e3248 ${step.pct_pm}%)`,
              }} />
    ...
    <div style={{ fontSize: 11, color: "#5a6480", marginTop: 8, padding: "0 4px" }}>
        ...
        return (
          <div key={b.brand} style={{
            ...
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 10 }}>
              <div style={{ fontSize: 16, fontWeight: 700, color: "#e2e8f0" }}>{b.brand}</div>
              <div style={{
                background: `${color}22`,
                color,
            ...
            {awareness && (
              <div style={{ borderTop: "1px solid #2e3248", paddingTop: 10, display: "flex", gap: 16 }}>
                <div>
                  <div style={{ fontSize: 18, fontWeight: 700, color }}>{awareness.value}</div>
                  <div style={{ fontSize: 10, color: "#5a6480" }}>언급 수</div>
                </div>
                <div>
                  <div style={{ fontSize: 18, fontWeight: 700, color: "#8892a4" }}>{awareness.pct}%</div>
                  <div style={{ fontSize: 10, color: "#5a6480" }}>인지율</div>
                </div>
                <div>
                  <div style={{ fontSize: 13, fontWeight: 600, color: "#a78bfa" }}>
                    F:{awareness.f} / M:{awareness.m}
                  </div>
                  <div style={{ fontSize: 10, color: "#5a6480" }}>성별 분포</div>
                </div>
              </div>
            )}
          </div>
...
// ── Insight card ──────────────────────────────────────────────────────────────
const Insight = ({ num, title, body, tag }) => (
  <div style={{
    ...S.card,
    display: "flex",
    gap: 20,
    ...
    <div>
      ...
      <div style={{ fontSize: 13, color: "#8892a4", lineHeight: 1.7 }}>{body}</div>
    </div>
  </div>
);
...
const dotColor = (r) => {
  if (r.gender === 'M') return COLORS.male;
  if (r.involvement === 'High') return COLORS.secondary;
  if (r.involvement === 'Low') return '#5a6480';
...
const FilterBtn = ({ label, active, onClick }) => (
  <button onClick={onClick} style={{
    ...
    borderColor: active ? "#6c7aff" : "#2e3248",
    background: active ? "#6c7aff22" : "transparent",
    color: active ? "#6c7aff" : "#8892a4",
  ...
  return (
    <div>
      ...
      <div style={{ display: "flex", gap: 6, flexWrap: "wrap", marginBottom: 20, alignItems: "center" }}>
        <span style={{ fontSize: 11, color: "#5a6480", marginRight: 2 }}>성별</span>
        {[["전체","전체"],["여성","F"],["남성","M"]].map(([lbl, val]) => (
          <FilterBtn key={val} label={lbl} active={filters.gender === val} onClick={() => setFilter("gender", val)} />
        ))}
        <span style={{ fontSize: 11, color: "#5a6480", marginLeft: 8, marginRight: 2 }}>관여도</span>
        ...
        <span style={{ fontSize: 11, color: "#5a6480", marginLeft: 8, marginRight: 2 }}>피부</span>
        {[["전체","전체"],["건성","건성"],["복합성","복합성"]].map(([lbl, val]) => (
          <FilterBtn key={val} label={lbl} active={filters.skinType === val} onClick={() => setFilter("skinType", val)} />
        ))}
        <span style={{ fontSize: 11, color: "#5a6480", marginLeft: "auto" }}>n={filtered.length}</span>
      </div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: selected ? "1fr 300px" : "1fr", gap: 20, alignItems: "start" }}>
        ...
        <div style={{ ...S.card, position: "relative" }}>
          {/* Y-axis end labels */}
          <div style={{ position: "absolute", top: 18, left: 30, fontSize: 10, color: "#5a6480", pointerEvents: "none" }}>탐색 의향 ↑</div>
          <div style={{ position: "absolute", bottom: 60, left: 30, fontSize: 10, color: "#5a6480", pointerEvents: "none" }}>안정적 사용 ↓</div>
          <ResponsiveContainer width="100%" height={420}>
            <ScatterChart margin={{ top: 16, right: 24, bottom: 48, left: 44 }}>
              <CartesianGrid strokeDasharray="3 3" stroke="#2e3248" />
              <XAxis type="number" dataKey="x" domain={[0.5, 5.5]} name="루틴 복잡도"
                tick={{ fill: "#5a6480", fontSize: 10 }} axisLine={false} tickLine={false}
                tickFormatter={(v) => ({ 1: "1단계", 2: "2단계", 3: "3단계", 4: "4단계", 5: "5단계+" })[Math.round(v)] || ""} />
              <YAxis type="number" dataKey="y" domain={[0, 5]} name="탐색 성향"
                tick={{ fill: "#5a6480", fontSize: 10 }} axisLine={false} tickLine={false} />
              <ZAxis range={[30, 30]} />
              <Tooltip cursor={{ strokeDasharray: "3 3", stroke: "#2e3248" }}
                content={({ active, payload }) => {
                  if (!active || !payload?.length) return null;
                  const r = payload[0]?.payload;
                  if (!r) return null;
                  return (
                    <div style={{ background: "#1a1d27", border: "1px solid #2e3248", borderRadius: 8, padding: "10px 14px", fontSize: 12, maxWidth: 210 }}>
                      <div style={{ color: "#e2e8f0", fontWeight: 600, marginBottom: 4 }}>응답자 #{r.id}</div>
                      <div style={{ color: "#8892a4" }}>{r.gender === 'F' ? '여성' : '남성'} · {r.skinType} · {r.involvement}</div>
                      <div style={{ color: "#6c7aff", marginTop: 6, fontSize: 11, fontStyle: "italic" }}>{r.summary}</div>
                      <div style={{ color: "#5a6480", marginTop: 6, fontSize: 11 }}>클릭하여 상세 보기</div>
                    </div>
                  );
                }}
              />
              <Scatter
                ...
                shape={(props) => {
                  ...
                  const c = dotColor(r);
                  return (
                    <circle cx={cx} cy={cy} r={isSel ? 7 : 4}
                      fill={c} fillOpacity={isSel ? 1 : 0.65}
                      stroke={isSel ? "#fff" : c} strokeWidth={isSel ? 2 : 0}
          ...
          {/* X-axis end labels */}
          <div style={{ display: "flex", justifyContent: "space-between", paddingLeft: 44, paddingRight: 24, marginTop: -6, marginBottom: 10 }}>
            <span style={{ fontSize: 10, color: "#5a6480" }}>← 루틴 단순 (1단계)</span>
            <span style={{ fontSize: 10, color: "#5a6480" }}>(5단계+) 루틴 복잡 →</span>
          </div>
          {/* Legend */}
          <div style={{ display: "flex", gap: 14, flexWrap: "wrap", paddingTop: 4 }}>
            {[
              { color: COLORS.secondary, label: "여성 · 고관여" },
              { color: COLORS.primary,   label: "여성 · 중관여" },
              { color: "#5a6480",        label: "여성 · 저관여" },
              { color: COLORS.male,      label: "남성" },
            ].map(({ color, label }) => (
        ...
        {selected && (
          <div style={{ ...S.card, borderTop: `2px solid ${dotColor(selected)}` }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 14 }}>
              <div>
                <div style={{ fontSize: 14, fontWeight: 700, color: "#e2e8f0" }}>응답자 #{selected.id}</div>
                <div style={{ fontSize: 12, color: "#8892a4", marginTop: 2 }}>
                  {selected.gender === 'F' ? '여성' : '남성'} · {selected.skinType} · {selected.steps}단계 루틴
                </div>
              </div>
              <button onClick={() => setSelected(null)}
                style={{ background: "none", border: "none", color: "#5a6480", cursor: "pointer", fontSize: 18, lineHeight: 1 }}>
                ×
              </button>
            </div>

            <div style={{ fontSize: 13, color: "#6c7aff", fontStyle: "italic", marginBottom: 16, lineHeight: 1.6, padding: "10px 12px", background: "#1a1f35", borderRadius: 6 }}>
              "{selected.summary}"
            </div>
            ...
            {[
              ...
              <div key={label} style={{ padding: "8px 0", borderBottom: "1px solid #2e3248", display: "flex", gap: 12 }}>
                <div style={{ fontSize: 11, color: "#5a6480", minWidth: 76, flexShrink: 0 }}>{label}</div>
                <div style={{ fontSize: 12, color: "#e2e8f0" }}>{value}</div>
              </div>
            ))}

            <div style={{ marginTop: 14, padding: "10px 12px", background: "#22263a", borderRadius: 6 }}>
              <div style={{ fontSize: 11, color: "#5a6480", marginBottom: 4 }}>분포 위치</div>
              <div style={{ fontSize: 12, color: "#8892a4" }}>
                루틴 복잡도 <b style={{ color: "#e2e8f0" }}>{selected.x}/5</b> · 탐색 성향 <b style={{ color: "#e2e8f0" }}>{selected.y}/5</b>
              </div>
            </div>
          </div>
        )}
      ...
      <div style={{ ...S.card, marginTop: 20 }}>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
          {[
            { tag: "고관여 여성 군집", text: "고관여 여성(보라색)은 우상단에 집중 — 루틴이 복잡하고 탐색 의향도 높은 이중 특성. 신제품·기능성 성분 정보에 가장 민감하게 반응하는 세그먼트." },
            { tag: "남성 세그먼트",   text: "남성(초록) 대다수가 좌하단 클러스터 형성. 루틴 단순 + 안정적 사용 패턴. 파트너 영향 없이는 루틴 변화 동인이 약하며, 기존 제품에 대한 고착도 높음." },
            { tag: "저관여 여성",     text: "저관여 여성(회색)은 1~2단계 루틴에 분포하지만 탐색 성향은 남성보다 상대적으로 높음. 적절한 트리거(지인 추천·SNS)가 루틴 진입 가능성을 열어두고 있음." },
            { tag: "대각선 분포",     text: "루틴 복잡도 ↑ → 탐색 성향 ↑의 양의 상관 패턴 관찰. 단, 3단계 루틴 그룹 내 탐색 성향 편차가 크게 나타남 — 루틴 단계 수만으로 세그먼트를 단순화하기 어렵다는 근거." },
          ].map(({ tag, text }, i) => (
            <div key={i} style={{ display: "flex", gap: 12, alignItems: "flex-start" }}>
              <div style={{ ...S.tag, flexShrink: 0, marginBottom: 0, whiteSpace: "nowrap" }}>{tag}</div>
              <div style={{ fontSize: 13, color: "#8892a4", lineHeight: 1.75 }}>{text}</div>
            </div>
          ))}
        </div>
      </div>
  ...
  return (
    <div>
      ...
      <div style={{ display: "flex", gap: 8, marginBottom: 20 }}>
        ...
        <span style={{ fontSize: 11, color: "#5a6480", alignSelf: "center", marginLeft: 8 }}>클릭하여 패턴 상세 보기</span>
      </div>
                    ...
                    <div style={{ display: "flex", alignItems: "center", gap: 4, flexWrap: "wrap" }}>
                      {p.steps.map((step, i) => (
                        <span key={i} style={{ display: "flex", alignItems: "center", gap: 4 }}>
                          ...
                          {i < p.steps.length - 1 && <span style={{ color: "#2e3248", fontSize: 12 }}>→</span>}
                        </span>
                      ))}
                    </div>
                  </div>
                  <div style={{ fontSize: 12, color: "#8892a4" }}>{p.desc}</div>
                </div>
                <div style={{ textAlign: "right", flexShrink: 0, minWidth: 56 }}>
                  <div style={{ fontSize: 18, fontWeight: 700, color: p.color }}>{p.pct}%</div>
                  <div style={{ fontSize: 11, color: "#5a6480" }}>n={p.count}</div>
                </div>
                <div style={{ color: "#5a6480", fontSize: 12, flexShrink: 0 }}>{isOpen ? "▲" : "▼"}</div>
              </div>

              {/* Expanded */}
              {isOpen && (
                <div style={{ marginTop: 16, paddingTop: 16, borderTop: "1px solid #2e3248", display: "grid", gridTemplateColumns: "1fr 1fr", gap: 20 }}>
                  <div>
                    <div style={{ fontSize: 11, color: "#5a6480", marginBottom: 6, textTransform: "uppercase", letterSpacing: "0.06em" }}>응답자 특성</div>
                    <div style={{ fontSize: 12, color: "#e2e8f0", lineHeight: 1.8, marginBottom: 12 }}>{p.profile}</div>
                    <div style={{ fontSize: 11, color: "#5a6480", marginBottom: 6, textTransform: "uppercase", letterSpacing: "0.06em" }}>주요 동기</div>
                    <div style={{ fontSize: 12, color: "#8892a4", lineHeight: 1.7 }}>{p.motivation}</div>
                  </div>
                  <div>
                    <div style={{ fontSize: 11, color: "#5a6480", marginBottom: 8, textTransform: "uppercase", letterSpacing: "0.06em" }}>주요 제품군</div>
                    <div style={{ display: "flex", flexWrap: "wrap", gap: 6, marginBottom: 14 }}>
                      {p.products.map((prod, i) => (
                        <span key={i} style={{ padding: "3px 9px", background: "#22263a", border: "1px solid #2e3248", borderRadius: 3, fontSize: 11, color: "#8892a4" }}>{prod}</span>
                      ))}
                    </div>
                    <div style={{ fontSize: 11, color: "#5a6480", marginBottom: 8, textTransform: "uppercase", letterSpacing: "0.06em" }}>주요 구매 계기</div>
...
// ── VOC ───────────────────────────────────────────────────────────────────────
const VOCCard = ({ quote, meta }) => (
  <div style={{ borderLeft: "3px solid #2e3248", paddingLeft: 16 }}>
    <div style={{ fontSize: 13, color: "#cbd5e1", lineHeight: 1.8, fontStyle: "italic", marginBottom: 8 }}>
      "{quote}"
    </div>
    <div style={{ fontSize: 11, color: "#5a6480" }}>{meta}</div>
  </div>
);

const VOCBlock = ({ vocs }) => (
  <div style={{ ...S.card, marginTop: 16, borderTop: "2px solid #2e3248" }}>
    <div style={{ ...S.cardTitle, marginBottom: 16 }}>응답자 목소리 (VOC)</div>
    <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
      {vocs.map((v, i) => <VOCCard key={i} {...v} />)}
    </div>
  </div>
);
    ...
    <div style={{ background: "#0f1117", minHeight: "100vh" }}>
      ...
      <div style={S.page}>
        ...
        <div style={S.header}>
          ...
          <div style={{ fontSize: 15, color: "#8892a4" }}>
        ...
        <Section id="respondents" title="응답자 분포 맵"
          ...
          <div style={{ ...S.card, marginBottom: 20, padding: "14px 20px", borderLeft: "3px solid #6c7aff" }}>
            <div style={{ fontSize: 13, color: "#8892a4", lineHeight: 1.8 }}>
              <b style={{ color: "#e2e8f0" }}>X축 (루틴 복잡도)</b>는 응답자가 보고한 루틴 단계 수를 기반으로 산출.{" "}
              <b style={{ color: "#e2e8f0" }}>Y축 (탐색 성향)</b>은 스킨케어 관여도·WTP 응답에서 파생한 추정치입니다.
            ...
            <div style={S.card}>
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 12, marginTop: 8 }}>
                {[
                  ...
                  <div key={i} style={{ display: "flex", gap: 14, alignItems: "flex-start", padding: "12px 0", borderBottom: "1px solid #2e3248" }}>
                    <div style={{ fontSize: 20 }}>{item.icon}</div>
                    <div style={{ flex: 1 }}>
                      <div style={{ fontSize: 13, fontWeight: 600, color: "#e2e8f0" }}>{item.title}</div>
                      <div style={{ fontSize: 12, color: "#8892a4", marginTop: 2 }}>{item.desc}</div>
                    </div>
        ...
        <Section id="brands" title="브랜드 인지도 및 연상"
          ...
          <div style={S.grid2}>
            ...
            <div style={S.card}>
              ...
              <div style={{ fontSize: 13, color: "#8892a4", lineHeight: 2, marginBottom: 16 }}>
                상위 브랜드들은 올리브영 중심 구매 채널과 SNS 광고 노출을 통해 인지되는 경향
              </div>
              <div style={{ ...S.card, background: "#22263a", padding: 16, marginBottom: 12 }}>
                <div style={{ fontSize: 12, fontWeight: 600, color: "#a78bfa", marginBottom: 6 }}>관여도 High 응답자 (n≈108)</div>
                <div style={{ fontSize: 12, color: "#8892a4" }}>평균 8–14개 브랜드 자발 상기. 드럭스토어·더마 라인 혼재</div>
              </div>
              <div style={{ ...S.card, background: "#22263a", padding: 16, marginBottom: 12 }}>
                <div style={{ fontSize: 12, fontWeight: 600, color: "#34d399", marginBottom: 6 }}>관여도 Low 응답자 — 남성 포함 (n≈97)</div>
                <div style={{ fontSize: 12, color: "#8892a4" }}>평균 2–4개 브랜드 상기. 현재 사용 제품 브랜드 1–2개에 집중</div>
              </div>
              <div style={{ ...S.card, background: "#22263a", padding: 16 }}>
                ...
                <div style={{ fontSize: 12, color: "#8892a4" }}>12개 AP 브랜드 중 1개 이상 언급 응답자 비율 (추정)</div>
              </div>
            </div>
          </div>
          <VOCBlock vocs={[
            { quote: "에스트라는 올리브영 갈 때마다 눈에 띄어서요. 민감성 피부에 좋다는 얘기를 들었고, 한 번 써보니까 자극이 없어서 계속 쓰게 됐어요.", meta: "여성 · 중관여 · 건성 · 응답자 #57" },
            { quote: "솔직히 브랜드는 잘 모르겠고... 지금 쓰는 것도 이름을 잘 모르는 것 같아요. 여자친구가 사다 준 거라서요.", meta: "남성 · 저관여 · 응답자 #194" },
          ]} />
        </Section>
          ...
          <div style={S.grid2}>
            <div style={S.card}>
              ...
              <StackedBarH
                ...
                height={260}
              />
              <div style={{ fontSize: 11, color: "#5a6480", marginTop: 8 }}>
            ...
            <div style={S.card}>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>
                {[
                  ...
                  <div key={i} style={{ background: "#22263a", borderRadius: 6, padding: "10px 14px" }}>
                    <div style={{ fontSize: 11, color: "#5a6480" }}>{s.label}</div>
          ...
          <div style={{ ...S.grid2, marginBottom: 20 }}>
            <div style={S.card}>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10, marginTop: 16 }}>
                <div style={{ background: "#22263a", borderRadius: 6, padding: "10px 14px" }}>
                  <div style={{ fontSize: 11, color: "#5a6480" }}>여성 평균 단계수</div>
                ...
                <div style={{ background: "#22263a", borderRadius: 6, padding: "10px 14px" }}>
                  <div style={{ fontSize: 11, color: "#5a6480" }}>남성 평균 단계수</div>
            ...
            <div style={S.card}>
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
                {[
                  ...
                  <div key={i} style={{ display: "flex", gap: 12, alignItems: "flex-start", padding: "12px 14px", background: "#22263a", borderRadius: 8 }}>
                    ...
                    <div>
                      ...
                      <div style={{ fontSize: 11, color: "#8892a4", marginTop: 3, lineHeight: 1.6 }}>{item.note}</div>
                    </div>
                  </div>
                ))}
              </div>
          ...
          <div style={S.card}>
            ...
            <div style={{ marginBottom: 16, display: "flex", gap: 20 }}>
              <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                ...
                <span style={{ fontSize: 12, color: "#8892a4" }}>AM 루틴</span>
              </div>
              <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                <div style={{ width: 32, height: 8, background: "#6c7aff", borderRadius: 4 }} />
                <span style={{ fontSize: 12, color: "#8892a4" }}>PM 루틴</span>
              </div>
            </div>
          ...
          <div style={{ ...S.card, marginBottom: 20 }}>
            ...
            <div style={{ display: "flex", gap: 16, marginBottom: 12 }}>
              <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
                ...
                <span style={{ fontSize: 11, color: "#8892a4" }}>Most Important 선택 횟수</span>
              </div>
              <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
                <div style={{ width: 12, height: 12, background: COLORS.negative, borderRadius: 2 }} />
                <span style={{ fontSize: 11, color: "#8892a4" }}>Least Important 선택 횟수</span>
              </div>
            </div>
          ...
          <div style={S.grid2}>
            <div style={S.card}>
              ...
              {mdSorted.slice(0, 4).map((d, i) => (
                <div key={i} style={{ padding: "14px 0", borderBottom: "1px solid #2e3248", display: "flex", alignItems: "center", gap: 14 }}>
                  ...
                  <div style={{ flex: 1 }}>
                    ...
                    <div style={{ fontSize: 11, color: "#8892a4" }}>Most: {d.most} / Least: {d.least}</div>
                  </div>
          ...
          <div style={S.card}>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 20 }}>
              <div>
                ...
                {[
                  { b: "에스트라", note: "피부 장벽·더마 포지션. 민감성·건성 핵심 브랜드. 올리브영 접근성 ↑" },
                  { b: "이니스프리", note: "자연성·가성비 인식 확고. 10–20대 유입 브랜드로 인식" },
                  { b: "에뛰드", note: "색조 연계 브랜드. 스킨케어 독립 인식 낮은 편" },
                ].map((item, i) => (
                  <div key={i} style={{ padding: "10px 0", borderBottom: "1px solid #2e3248" }}>
                    <div style={{ fontSize: 13, fontWeight: 600, color: "#e2e8f0" }}>{item.b}</div>
                    <div style={{ fontSize: 12, color: "#8892a4" }}>{item.note}</div>
                  </div>
                ))}
              </div>
              <div>
                ...
                {[
                  { b: "라네즈", note: "수분·워터 이미지 있지만 인지 응답 적음. 트렌드 소비자 타겟 강화 여지" },
                  { b: "오휘·한율", note: "프리미엄·안티에이징 포지션이나 젊은 응답자 인지 낮음" },
                  { b: "헤라·비디비치", note: "색조 브랜드로 인식, 스킨케어 독립 포지셔닝 미약" },
                ].map((item, i) => (
                  <div key={i} style={{ padding: "10px 0", borderBottom: "1px solid #2e3248" }}>
                    <div style={{ fontSize: 13, fontWeight: 600, color: "#e2e8f0" }}>{item.b}</div>
                    <div style={{ fontSize: 12, color: "#8892a4" }}>{item.note}</div>
